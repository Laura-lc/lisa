...
Classmethod also work, so typeclasses can be used to define factory interfaces::
    ...
    class FromString(TypeClass):
        "Build a value out by parsing a string"
    ...
    assert 33 == FromString(int).from_string('33')
...
instance of either :class:`lisa.typeclass.ValueProxy` or
:class:`lisa.typeclass.TypeProxy` is returned.
...
class TypeClassMeta(type):
    ...
    def __new__(cls, name, bases, dct, *args, types=None, **kwargs):
        ...
        # Someone tries to inherit from the typeclass to make an instance
            ...
            # will still live a secrete life internally for error reporting
            # though.
            return None
...
class TypeClass(metaclass=TypeClassMeta):
    ...
    def __new__(cls, obj):
        ...
        # __slots__).
            ...
            proxy_cls = TypeProxy if isinstance(obj, type) else ValueProxy
            new_obj = proxy_cls(obj, dct)
...
class TypeProxy(ValueProxy):
    """
    Values of this class are returned when casting a type to a typeclass.

    It implements the modified attribute lookup, so we can use the typeclass
    methods. All other attribute lookups will go through untouched.

    Additionally, the ``__call__`` magic method is correctly redirected to
    allow implementing classmethod.
    """
    def wrap(name):
        def wrapper(self, *args, **kwargs):
            # Use the plain getattribute, not the one from ValueProxy
            obj = super(ValueProxy, self).__getattribute__('_obj')
            meth = getattr(obj, name)
            return meth(*args, **kwargs)
        return wrapper

    # Allow calling the class as usual, which is necessary to use factory
    # classmethod that return new instances (alternative constructors).
    __call__ = wrap('__call__')
    del wrap
